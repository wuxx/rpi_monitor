...
int output_init(output_parameter *param, int id)
{
    ...
    if  (mjpgFileName == NULL) {
...
#if 0
        {
            ...
            strftime(curr_date, sizeof(curr_date), "%F", newtime);
            if (strncmp(last_date, curr_date, sizeof(curr_date)) != 0) {

                path = malloc(strlen(folder) + 3 + sizeof(curr_date));

                sprintf(path, "%s/%s", folder, curr_date);
                OPRINT("mkdir [%s] ", path);
                ...
                memcpy(last_date, curr_date, sizeof(curr_date));
        ...
        char *fnBuffer = malloc(strlen(mjpgFileName) + strlen(folder) + 3 + sizeof(curr_date));


        sprintf(fnBuffer, "%s/%s/%s", folder, curr_date, mjpgFileName);
#endif
